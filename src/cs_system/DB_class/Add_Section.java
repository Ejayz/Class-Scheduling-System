...
public class Add_Section {
    ...
    public boolean ChoiceLoad(String day, JComboBox end) {
        String sql = "Select * from room where DAY=? AND IS_USED_ROOM=? AND IS_EXIST=? ";
        ...
        try {
            ...
            while (rs.next()) {
                if (rs.getString("SECTION").contains(search)
                        || rs.getString("SUBJECT_CODE").contains(search)
                        || rs.getString("DESCRIPTION").contains(search)
                        || rs.getString("DEPARTMENT").contains(search)
                        || rs.getString("SUNITS").contains(search)
                        || rs.getString("TUNITS").contains(search)
                        ...
                        || rs.getString("STUDENT_NUMBR").contains(search)
                        || rs.getString("REMARK").contains(search)) {
                    result = true;
                    dm.addRow(new Object[]{id++, rs.getString("SECTION"), rs.getString("SUBJECT_CODE"), rs.getString("DESCRIPTION"), rs.getString("DEPARTMENT"), rs.getString("SUNITS"), rs.getString("TUNITS"), rs.getString("DAY"), rs.getString("START"), rs.getString("END"), rs.getString("ROOM"), rs.getString("STUDENT_NUMBR"), rs.getString("REMARK")});
